...
| [opus-orchestration](skills/opus-orchestration/) | Large multi-deliverable builds as workflow-orchestrated Opus fleets: the premium main thread plans, gates, and reviews; Opus agents research, draft, adversarially verify, and fix | A goal-style one-pass prompt; Claude Code's Workflow tool |
| [clodex](skills/clodex/) | The front door to a `plan → build → verify → ship` workflow for Claude Code + Codex CLI: preflight, resuming an interrupted run, the repo's `.clodex/profile.json`, and the change boundary a run starts from — routes the feature and audit lanes | The five `clodex-*` stage skills, installed beside it; a logged-in Codex CLI |
...
| [clodex-audit](skills/clodex-audit/) | The audit lane: read-only investigation with every claim tagged `VERIFIED (method)` or `HYPOTHESIS`, a verdict-first three-bucket report, and per-item routing that feeds follow-on runs | `clodex`, which routes audit-shaped asks into it; the core-path skills its routing hands work to |
| [dialectical-review](skills/dialectical-review/) | Pressure-testing a strategy or belief in one reasoning pass: state it as its strongest thesis, build the real antithesis, find the synthesis that survives both | `empirical-falsification` for the fact-shaped version of "is this actually right"; `red-team-pressure-test` for the execution-risk version |
| [empirical-falsification](skills/empirical-falsification/) | Checking a claim against real WebSearch evidence instead of recalled confidence, following citation chains to a primary source or two dead ends, and reporting holds/falsified/partially-true | `dialectical-review` for positions instead of facts; feeds the fact layer underneath a `red-team-pressure-test` pass |
| [red-team-pressure-test](skills/red-team-pressure-test/) | Dispatching a genuinely independent adversarial subagent to find every gap and weak assumption in a plan before a client or reality does, for anything without a dedicated review lane already | `dialectical-review` and `empirical-falsification` upstream; a `clodex`/TRIP-style pipeline or `codex-plan-review` instead, if what's under test is code |
